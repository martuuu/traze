...
function Avatar({ src, alt, fallback }: { src?: string, alt: string, fallback: string }) {
    return (
        <div className="h-10 w-10 rounded-full overflow-hidden bg-sand-gold-50 border border-border flex items-center justify-center shrink-0">
            {src ? (
                 /* eslint-disable-next-line @next/next/no-img-element */
                <img src={src} alt={alt} className="h-full w-full object-cover" />
            ) : (
                <span className="font-bold text-muted-foreground font-light text-sm">{fallback}</span>
            )}
        </div>
    )
}
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-foreground font-title tracking-tight">Pacientes</h1>
                    <p className="text-muted-foreground font-light">Gestión de usuarios medicinales y sus cultivos asignados.</p>
                </div>
                <Button 
            ...
            <Card className="border-border shadow-sm overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-muted-foreground font-light uppercase bg-card font-bold">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-50 bg-white">
                            {filteredPatients.map((patient) => (
                                <tr key={patient.id} className="hover:bg-card transition-colors group">
                                    <td className="p-4">
                                        <input 
                                            type="checkbox" 
                                            className="rounded border-slate-300 text-pastel-green-600 focus:ring-pastel-green-500"
                                    ...
                                    <td className="px-6 py-4">
                                        <Link href={`/dashboard/patients/${patient.id}`} className="flex items-center gap-3">
                                            ...
                                            />
                                            <div>
                                                <div className="font-bold text-foreground font-title group-hover:text-pastel-green-600 transition-colors">{patient.name}</div>
                                                <div className="text-xs text-muted-foreground font-light">Alta: 12/03/2023</div>
                                            </div>
                                        </Link>
                                    </td>
                                    <td className="px-6 py-4">
                                        <Badge className={patient.status === 'active' ? 'bg-pastel-green-100 text-pastel-green-700 hover:bg-pastel-green-100' : 'bg-sand-gold-50 text-muted-foreground font-light'}>
                                            {patient.status === 'active' ? 'Activo' : 'Inactivo'}
                                        </Badge>
                                    </td>
                                    <td className="px-6 py-4 font-mono text-muted-foreground font-light text-xs">
                                        {patient.id}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1">
                                            <span className="font-bold text-foreground font-title">{patient.plants ? patient.plants.length : 0}</span>
                                            <span className="text-muted-foreground font-light text-xs">/ 9</span>
                                        </div>
                                        <div className="w-24 h-1.5 bg-sand-gold-50 rounded-full mt-1 overflow-hidden">
                                            <div className="h-full bg-pastel-green-500 rounded-full" style={{ width: `${((patient.plants ? patient.plants.length : 0) / 9) * 100}%` }}></div>
                                        </div>
                                    </td>
            ...
            >
                <div className="space-y-4">
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-foreground font-medium block">Nombre Completo</label>
                        <Input 
                     ...
                     <div className="space-y-2">
                        <label className="text-sm font-bold text-foreground font-medium block">DNI / Pasaporte</label>
                        <Input placeholder="Ej. 30123456" />
                    </div>
                     <div className="space-y-2">
                        <label className="text-sm font-bold text-foreground font-medium block">N° REPROCANN</label>
